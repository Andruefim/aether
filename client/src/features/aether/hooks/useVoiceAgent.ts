 ...
 */
export function useVoiceAgent({ captureScreenshot, onUiReady }: UseVoiceAgentOptions) {
  ...
  const recordWithSilenceDetection = useCallback((): Promise<Blob | null> => {
    return new Promise(async (resolve) => {
      ...
      const SILENCE_DURATION_MS = 1500;   // 1.5s silence → stop
      const MAX_DURATION_MS = 30_000;     // safety limit 30s
      ...
      const checkSilence = () => {
        ...
        const silentMs = Date.now() - lastSoundAt;
            ...
            switch (event.type) {
              case 'speak':
                if (event.text) {
                  ...
                  // Play TTS (don't await — let it run while we continue reading SSE)
  ...
  const runCycle = useCallback(async () => {
    ...
    // Wait for TTS to finish before next cycle
