...
import { useGoalSummaries } from '../hooks/useGoalSummaries';

function ProgressBar({ value, color = '#a78bfa' }: { value: number; color?: string }) {
  return (
    <div style={{ height: 4, borderRadius: 2, background: 'rgba(255,255,255,0.08)', overflow: 'hidden', flexShrink: 0 }}>
      <div
        style={{
...
export function ResearchSummaryWidget() {
  const { summaries, loading, refresh } = useGoalSummaries({ autoRefreshOnWake: true });
  const [activeTab, setActiveTab] = useState<string | null>(null);
  const [collapsed, setCollapsed] = useState(false);

  const current = (activeTab ? summaries.find((s) => s.goalId === activeTab) : summaries[0]) ?? null;
      ...
      {!collapsed && (
        <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
          ...
          {summaries.length > 1 && (
            ...
            >
              {summaries.map((s) => {
                const isActive = (activeTab ?? summaries[0]?.goalId) === s.goalId;
                return (
                  <button
                    key={s.goalId}
                    ...
                    style={{
                      ...
                      color:        isActive ? '#fde68a' : '#6b7280',
                      background:   isActive ? 'rgba(251,191,36,0.10)' : 'none',
                      border:       'none',
                      borderBottom: isActive ? '2px solid #fde68a' : '2px solid transparent',
                      cursor:       'pointer',
                      whiteSpace:   'nowrap',
                      maxWidth:     100,
                      overflow:     'hidden',
          ...
          <div style={{ padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
            {loading && (
              <span style={{ color: '#6b7280', fontSize: 11, textAlign: 'center' }}>
                Generating summaries…
              </span>
            )}

            {!loading && summaries.length === 0 && (
              <span style={{ color: '#4b5563', fontSize: 11, textAlign: 'center' }}>
            ...
            {!loading && current && (
              <>
                ...
                {current.bullets.length > 0 && (
                  <ul style={{ margin: 0, padding: '0 0 0 14px', display: 'flex', flexDirection: 'column', gap: 5 }}>
                    {current.bullets.map((b, i) => (
                      <li key={i} style={{ color: '#e5e7eb', fontSize: 11, lineHeight: 1.5 }}>{b}</li>
                    ))}
                  </ul>
                )}
                ...
                <div style={{ color: '#4b5563', fontSize: 10, display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                  ...
                  <span title="Average novelty">novelty {(current.avgSurprise * 100).toFixed(0)}%</span>
